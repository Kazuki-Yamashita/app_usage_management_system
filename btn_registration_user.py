...
import user_registration_to_db as regUserdb #データベースを操作するファイル
import show_message as mes #メッセージボックスを表示するモジュール


#利用登録ボタンを押した際の処理
def btn_reg_user(user_name, user_ruby_name, user_reg_undergraduate_combobox,
 input_lab_name, input_id, input_new_password,
  input_confirm_password, is_check_new_lab, window):
    ...
    user_reg_undergraduate = user_reg_undergraduate_combobox.get() #選択した学部を取得
    lab_list = info.offer_lab_list(user_reg_undergraduate) #学部の研究室一覧を取得(リスト)
    user_reg_lab = input_lab_name.get() #選択した研究室・ゼミを取得
    ...
    is_reged_lab = conid.confirm_reged_lab(user_reg_lab)
    #「新規で研究室を登録」にチェックがついているか調べる
    is_new_lab = is_check_new_lab.get()

    #入力が適切か判定(適切であればTrue)
    result_input = isInpReg.is_input_entry_reg_user(reg_name, reg_ruby_name, user_reg_lab,
     lab_list, is_reged_lab, reg_id, new_password, confirm_password, is_equal_password,
    ...
    if result_input: #入力が適切な場合
        ...
        if registration_final_confirm:
            ...
            try: #データベースへ情報を追加する
                registrating = regUserdb.registration_user(reg_id, reg_name, reg_ruby_name,
                user_reg_undergraduate, user_reg_lab, new_password, reg_time)
                #登録完了の画面を表示
