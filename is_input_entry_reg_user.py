...
def is_input_entry_reg_user(user_name, user_ruby_name, reg_lab,
 ...
 is_equal_password, is_new_lab, input_new_password,
 input_confirm_password, window):
    ...
    #入力した名前の文字が適切でない場合
    elif not paw.pattern_name(user_name):
        mes.error("名前に使用できない文字があります", window)
        return False

    #名前にフリガナが適切でない場合
    elif not paw.pattern_ruby_name(user_ruby_name):
        mes.error("フリガナに使用できない文字があります", window)
        return False

    #入力したIDが適切でない場合
    elif not paw.pattern_id(id):
        mes.error("IDが適切ではありません", window)
        return False

    #入力したパスワードが適切でない場合
    elif not paw.pattern_password(new_password):
        mes.error("パスワードが適切ではありません", window)
        input_new_password.delete(0, tk.END) #入力したパスワードを削除する
        input_confirm_password.delete(0, tk.END) #入力した確認用パスワードを削除する
        return False
